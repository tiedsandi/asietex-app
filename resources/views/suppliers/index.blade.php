...
@section('content')
    <div class="bg-white rounded-xl shadow-sm overflow-hidden">
        <div class="px-5 py-4 flex items-center justify-between border-b border-gray-100">
            <h6 class="font-semibold text-gray-700">Daftar Supplier</h6>
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 text-xs text-gray-500 uppercase">
                    <tr>
                        ...
                        <th class="px-5 py-3 text-left font-medium">Nama Supplier</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($suppliers as $supplier)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-5 py-3 font-semibold text-gray-800">{{ $supplier->name }}</td>
                            <td class="px-5 py-3 text-gray-600">{{ $supplier->phone ?? '-' }}</td>
                            <td class="px-5 py-3 text-gray-600">{{ $supplier->email ?? '-' }}</td>
                            <td class="px-5 py-3 text-gray-500">{{ $supplier->address ?? '-' }}</td>
                            <td class="px-5 py-3">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-5 py-8 text-center text-gray-400">Belum ada data supplier.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
