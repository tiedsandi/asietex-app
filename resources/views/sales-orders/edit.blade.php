...
@section('page-title', 'Transaksi — Edit Sales Order')

@section('content')
    <form action="{{ route('sales-orders.update', $salesOrder) }}" method="POST" id="soForm">
        @csrf
        @method('PUT')
        <div class="space-y-4">
            <div class="bg-white rounded-xl shadow-sm overflow-hidden">
                <div class="px-5 py-4 border-b border-gray-100">
                    <h6 class="font-semibold text-gray-700">Informasi Sales Order — <span
                            class="text-gray-400 font-normal">{{ $salesOrder->so_number }}</span></h6>
                </div>
                <div class="p-5 grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Tanggal Order <span
                                class="text-red-500">*</span></label>
                        <input type="date" name="order_date" value="{{ old('order_date', $salesOrder->order_date) }}"
                            required class="{{ $inp }}">
                    </div>
                    <div>
                        ...
                        <select name="status" required class="{{ $inp }}">
                            @foreach (['pending', 'shipped', 'cancelled'] as $s)
                                <option value="{{ $s }}"
                                    {{ old('status', $salesOrder->status) == $s ? 'selected' : '' }}>{{ ucfirst($s) }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div class="sm:col-span-3">
                        ...
                        <textarea name="notes" rows="2" class="{{ $inp }}">{{ old('notes', $salesOrder->notes) }}</textarea>
                    </div>
                </div>
            </div>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody id="detailBody" class="divide-y divide-gray-50">
                            @foreach ($salesOrder->details as $i => $detail)
                                <tr id="row-{{ $i }}">
                                    <td class="px-4 py-2"><select name="details[{{ $i }}][product_id]"
                                    ...
                                    <td class="px-4 py-2"><input type="number"
                                            ...
                                            min="0" value="{{ $detail->unit_price }}" required
                                            oninput="calcSubtotal({{ $i }})"></td>
                        ...
                        <tfoot class="bg-gray-50">
                            <tr>
                                ...
                                <td class="px-4 py-3 text-right font-bold text-gray-800" id="grandTotal">Rp
...
@section('scripts')
    <script>
        let rowCount = {{ $salesOrder->details->count() }};
