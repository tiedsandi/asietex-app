...
@section('page-title', 'Master Data — Edit Customer')

@section('content')
    <div class="bg-white rounded-xl shadow-sm overflow-hidden max-w-xl">
        <div class="px-5 py-4 border-b border-gray-100">
            <h6 class="font-semibold text-gray-700">Form Edit Customer</h6>
        </div>
        <div class="p-5">
            <form action="{{ route('customers.update', $customer) }}" method="POST" id="theForm">
                @csrf
                @method('PUT')
                ...
                <div class="mb-5">
                    ...
                    <textarea name="address" rows="3" class="{{ $inp }}">{{ old('address', $customer->address) }}</textarea>
                </div>
